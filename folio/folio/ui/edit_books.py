...
from folio.scanner import embed_cover_in_epub
...
class EditPage(Gtk.Box):
    ...
    def _on_save(self, _btn):
        ...
        def _bg():
            update_book(book_id, title=title, year=year, pages=pages,
                        description=desc, series=series, series_num=series_num)
            ...
            if cover_bytes:
                ...
                try:
                    book = get_book(book_id)
                    if book and book.get("epub_path"):
                        from pathlib import Path as _Path
                        embed_cover_in_epub(_Path(book["epub_path"]), cover_bytes)
                except Exception:
                    pass
